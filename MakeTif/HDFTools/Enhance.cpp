...
void Enhance::Cast16To8(ushort *src,uchar* des,int width,int height)
{
	...
	for(int i=0;i<len;++i)
		...
		for(int i=0;i<len;++i)
		{
			ushort temp=(*pData)*255.0/4095.0+0.5;
			if(temp>max_val8)
				temp=max_val8;
			*pDest=(uchar)temp;
			pData++;
...
void  Enhance::Histogram_Equalization_IgnoreZero(ushort *src,int width,int height)
{
	//避免填充部分对对比度的影响
	// 指向源图像的指针

	int max_val=4095;//12位
	...
	for (i = 0; i <max_val+1; i++)
	{
		...
		// 计算对应的新灰度值
		int tp=(lTemp-lCount[0]) * (float)max_val / (height *width-lCount[0])+0.5f;
...
void  Enhance::Histogram_Equalization_IgnoreZero(uchar *src,int width,int height)
{
	//避免填充部分对对比度的影响
	// 指向源图像的指针
	const int max_val=255;//12位
	uchar*	lpSrc;
	// // 临时变量
	int	lTemp;
	// // 循环变量
	...
	int	j;
	// 灰度映射表
	uchar	*bMap=new uchar[max_val+1];
	memset(bMap,0, (max_val+1)*sizeof(uchar));
	// // 灰度映射表
	int	*lCount=new int[max_val+1];
	memset(lCount,0,(max_val+1)*sizeof(int));
